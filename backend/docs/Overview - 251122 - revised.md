...
- **Backend**
  ...
  - Exposes a REST endpoint `/rolling-put-candidates/{ticker}` that:
    ...
    - Flags each put with:
      - `meets_band` (inside band window),
      - `meets_delta` (|delta| ≤ max requested),
      - `meets_credit` (credit % ≥ min requested),
      - and a `type` (`opportunity`, `neighbor`, `incomplete`).
    - Returns a structured JSON payload consumed by the frontend.
  - Strategy defaults are read from `backend/strategy_defaults.env` (factory defaults for delta, credit, band distance, and credit ceiling).
...
- **Distance from lower band (+/- point)**
  ...
  - Example: distance = 3 means the band window is `[lower_band, lower_band + 3]`.
  - Slider:
    - Min: **1**
    - Max: **10**
    - Step: 1.
...
1. The frontend builds a `QueryParams` object with:
   ...
   - `delta_min` (fixed lower bound, currently 0.10)
   - `delta_max` (from Max Delta control)
   - `credit_min_pct` (from Min Credit control)
   - `credit_max_pct` (fixed at 0.02 = 2.0%)
   - `band_window` (from Distance control)
...
- **First pass – rule-based classification:**
  ...
  not meets_band      → neighbor (defensive fallback)
  meets_band + delta + credit → opportunity
  meets_band + (delta XOR credit) → candidate_strong
  meets_band only     → candidate
````
...
* **Score symbols:**
  ...
  * `candidate_*` and `neighbor` → empty score cell (no symbol).
...
Above the Opportunities card, the app displays the current **data source**, derived from the backend’s `data_provider` value:
